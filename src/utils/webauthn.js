...
const AAGUID = new Uint8Array([0xa2, 0x8c, 0x3d, 0xf7, 0x1b, 0x5e, 0x4a, 0x89,
                               0x9c, 0xd0, 0x2e, 0x6f, 0xb3, 0x41, 0x7a, 0xd8])

export async function makeCredential(options, origin) {
  ...
  const cosePublicKey = jwkToCoseKey(publicKeyJwk)
  const attestedCredData = buildAttestedCredentialData(credentialId, cosePublicKey)

  const rpIdHash = await sha256(rpId)
  const flags = 0x41
  const signCount = 0
  const authData = buildAuthData(rpIdHash, flags, signCount, attestedCredData)
  ...
  const challengeBase64url = (challengeBuf instanceof ArrayBuffer)
    ? arrayBufferToBase64url(challengeBuf)
    : String(challengeBuf)
  ...
  const credential = {
    id: arrayBufferToBase64url(credentialId.buffer),
  ...
  response.toJSON = function() {
    return {
      challenge: challengeBase64url,
      origin: origin,
      clientDataJSON: arrayBufferToBase64url(clientDataJSONBuffer.buffer),
      attestationObject: arrayBufferToBase64url(attestationObject),
      transports: ['internal'],
      getPublicKeyAlgorithm: -7,
      getTransports: ['internal'],
      getAuthenticatorData: arrayBufferToBase64url(authData.buffer),
      getPublicKey: arrayBufferToBase64url(new Uint8Array(CBOR.encode(cosePublicKey)).buffer),
    }
  }
  ...
  const userIdBase64url = arrayBufferToBase64url(
...
export async function getAssertion(options, origin, storedCredentials) {
  ...
  const authData = buildAuthData(rpIdHash, flags, signCount, null)
  ...
  const signatureBase = concatBuffers(authData.buffer, clientDataHash)
  ...
  const rawSignature = p1363ToDer(new Uint8Array(p1363Signature)).buffer

  const credentialIdBytes = base64urlToArrayBuffer(matchedCredential.credentialId)
  const userHandle = matchedCredential.userId
    ? base64urlToUint8Array(matchedCredential.userId)
  ...
  credential.response.toJSON = function() {
    return {
      ...
      userHandle: userHandle && userHandle.length > 0 ? arrayBufferToBase64url(userHandle.buffer) : null,
    }
  }
...
export function serializeCredential(credential) {
  ...
  if (resp.clientDataJSON) {
    result.response.clientDataJSON = resp.clientDataJSON instanceof ArrayBuffer
      ? arrayBufferToBase64url(resp.clientDataJSON)
      : resp.clientDataJSON
  }
  if (resp.authenticatorData) {
    result.response.authenticatorData = resp.authenticatorData instanceof ArrayBuffer
      ? arrayBufferToBase64url(resp.authenticatorData)
      : resp.authenticatorData
  }
  if (resp.signature) {
    result.response.signature = resp.signature instanceof ArrayBuffer
      ? arrayBufferToBase64url(resp.signature)
      : resp.signature
  }
  if (resp.attestationObject) {
    result.response.attestationObject = resp.attestationObject instanceof ArrayBuffer
      ? arrayBufferToBase64url(resp.attestationObject)
      : resp.attestationObject
  }
  if (resp.userHandle) {
    result.response.userHandle = resp.userHandle instanceof ArrayBuffer
      ? arrayBufferToBase64url(resp.userHandle)
      : resp.userHandle
  }
  if (resp.transports) {
    result.response.transports = resp.transports
  }

  // getter 方法（序列化为字符串）
  if (resp.getAuthenticatorData) {
    const ad = resp.getAuthenticatorData()
    result.response.getAuthenticatorData = ad instanceof ArrayBuffer
      ? arrayBufferToBase64url(ad)
      : ad
  }
  if (resp.getPublicKey) {
    const pk = resp.getPublicKey()
    result.response.getPublicKey = pk instanceof ArrayBuffer
      ? arrayBufferToBase64url(pk)
      : pk
  }
  if (resp.getPublicKeyAlgorithm !== undefined) {
    result.response.getPublicKeyAlgorithm = typeof resp.getPublicKeyAlgorithm === 'function'
      ? resp.getPublicKeyAlgorithm()
      : resp.getPublicKeyAlgorithm
  }
  if (resp.getTransports) {
    result.response.getTransports = typeof resp.getTransports === 'function'
      ? resp.getTransports()
      : resp.getTransports
  }
