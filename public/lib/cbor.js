// ============================================
// CBOR 编码器（简化版，用于 WebAuthn attestation object）
...
class CBOREncoder {
  ...
  encodeUnsignedInt(value) {
    this.encodeHead(CBOR.UNSIGNED_INT, value);
  }

  encodeNegativeInt(value) {
    this.encodeHead(CBOR.NEGATIVE_INT, -(value + 1));
  }

  encodeFloat(value) {
    this.bytes.push((CBOR.UNSIGNED_INT << 5) | 27);
  ...
  encodeByteString(bytes) {
    this.encodeHead(CBOR.BYTE_STRING, bytes.length);
  ...
  encodeTextString(str) {
    ...
    this.encodeHead(CBOR.TEXT_STRING, encoded.length);
  ...
  encodeArray(arr) {
    this.encodeHead(CBOR.ARRAY, arr.length);
  ...
  encodeMap(obj) {
    ...
    this.encodeHead(CBOR.MAP, keys.length);
  ...
  encodeNull() {
    this.bytes.push((CBOR.UNSIGNED_INT << 5) | 22);
  }

  encodeTrue() {
    this.bytes.push((CBOR.UNSIGNED_INT << 5) | 21);
  }

  encodeFalse() {
    this.bytes.push((CBOR.UNSIGNED_INT << 5) | 20);
  }
}
